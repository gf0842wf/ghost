...
try:
    from PySide import QtGui, QtCore, QtNetwork, QtWebKit
except ImportError:
    try:
        from PyQt4 import QtGui, QtCore, QtNetwork, QtWebKit
    except ImportError:
        raise Exception('you need install PySide or PyQt4')
...
class Ghost(object):
    ...
    def __init__(
        ...
        qInstallMsgHandler(QTMessageProxy(logger))
        if plugin_path:
...
class Session(object):
    ...
    def wait_for(self, condition, timeout_message, timeout=None):
        ...
        while not condition():
            if time.time() > (started_at + timeout):
                raise GhostTimeoutError(timeout_message)
            self.sleep()
    ...
    def wait_for_page_loaded(self, timeout=None):
        ...
        if page:
            ...
            page.text = str(page.content).decode('UTF-8')
